...
1. [LRDM: Motion-Aware 4D Dynamic View Synthesis in One Second](https://github.com/chenguolin/LRDM)
2. [MoVieS: Motion-Aware 4D Dynamic View Synthesis in One Second](https://github.com/chenguolin/MoVieS)
3. [DiffSplat: Single-View 3D Reconstruction via Differentiable Splatting](https://chenguolin.github.io/projects/DiffSplat/)
4. [Wan: A Powerful Video Generation Model](https://github.com/Wan-Video/Wan2.1)
...
1. **CUDA Out of Memory**
   - Reduce `batch_size_per_gpu` in config
   - Enable gradient checkpointing (already enabled by default)
   - Use smaller input resolution

2. **Dataset Path Issues**
   - Ensure `DATASET_ROOT` environment variable is set
   - Or configure the root path in `src/options.py`

3. **Missing Model Weights**
   - Verify checkpoints are in correct directories
   - Check paths in `src/options.py`

4. **Import Errors**
   - Ensure all dependencies are installed: `pip install -r settings/requirements.txt`
...
│   │   ├── lrdm.py                # LRDM model (main 3DGS reconstruction)
│   │   ├── tiny_vae.py            # TinyVAE / TAEHV for latent alignment
│   │   ├── latent_alignment.py      # Latent alignment models
│   │   ├── wan.py                 # Wan video diffusion model
│   │   ├── networks/              # Network components
│   │   └── gs_render/             # 3D Gaussian splatting renderer
│   ├── data/                      # Dataset loaders
│   ├── train_wan_cc.py            # Camera control training
│   ├── train_latent_alignment.py   # Latent alignment training
│   ├── train_splatrecon.py         # LRDM training
│   ├── infer_nvs.py                 # Unified inference script
│   ├── preprocess_npz.py             # Data preprocessing
│   └── options.py                 # Configuration options
├── tests/                        # Test scripts
│   ├── test_environment.py        # Environment verification
│   ├── test_wan_model.py          # Wan model loading
│   ├── test_latent_alignment.py   # Latent alignment pipeline
│   └── README.md                 # Test documentation
├── configs/                       # Training configs
├── scripts/                       # Shell scripts
├── settings/                      # Environment setup
├── resources/                     # Checkpoints and resources
│   ├── ckpts/                    # Model checkpoints
│   │   ├── lrdm_ckpt.safetensors  # LRDM checkpoint
│   │   ├── lrdm_ckpt/            # LRDM training checkpoints
│   │   ├── Wan2.2-TI2V-5B/        # Wan base model
│   │   └── wan_cc_60000/          # Camera control checkpoint
│   └── negative_embed.npy
└── README.md
```

**Backward Compatible Aliases**:
- `SplatRecon` → `LRDM` (Python class alias in src/models/__init__.py)
- `movies` / `movies_static` → `lrdm` / `lrdm_static` (config aliases in src/options.py)
